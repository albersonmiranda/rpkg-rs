use crate::cli::{build_git_url, Cli};
use crate::cran::escape_r_string;

pub fn build_rscript_expr(
    args: &Cli,
    cran_repo: &str,
    ...
    let personal_library_expr = r#"local({
    lib <- .libPaths()[1]
    ...
    if library.is_none() {
        expr_parts.push(format!("personal_lib <- {}", personal_library_expr));
        expr_parts.push(
            r#"if (file.access(.libPaths()[1], 2) != 0) .libPaths(c(personal_lib, .libPaths()))"#
                .to_string(),
        );
    }
    ...
    if *update {
        ...
        if let Some(lib) = library {
            ...
            update_expr = format!(
                "update.packages(ask = FALSE, lib.loc = {}, repos = c({}))",
                personal_library_expr, repos_expr
            );
        }
